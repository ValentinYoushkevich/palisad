/**
 * Чистая логика сверки инвентаризации (§ «Инвентаризация», ФЭ5). Ни импортов из Dexie/http,
 * ни сайд-эффектов — модуль юнит-тестируется, а его алгоритм зеркалит бэкендовый
 * `src/utils/inventoryDiff.js` один-в-один (офлайн-предпросмотр расхождений до отправки на
 * сервер). Всё сравнение делается над уже отфильтрованными АКТИВНЫМИ растениями (фильтрацию
