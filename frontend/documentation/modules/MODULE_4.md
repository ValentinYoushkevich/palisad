...
- Раздел `Сотрудники` относится к блоку `Администрирование`.
- Пункт меню `Сотрудники` виден только роли `Owner`.
- Дополнительно к скрытию в меню, маршрут `/staff` должен быть защищён role-guard на уровне router (`owner` only).
...
| 7 | Не-owner не может открыть `/staff` по URL | Войти как `worker`, открыть `/staff` → редирект |
