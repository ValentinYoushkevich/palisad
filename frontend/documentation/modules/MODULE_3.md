...
- Пункт `Питомник` находится в `Общем доступе`, потому что экран настроек/контекста используется не только `Owner`.
- Owner-only действия внутри страницы (пример: управление подпиской) не переносят весь раздел в `Администрирование`.
- Для `sidebar`:
  - `Создать питомник` показывается при отсутствии контекста питомника;
  - `Питомник` показывается при существующем питомнике.
