...
- Раздел `Локации` относится к `Общему доступу` (просмотр доступен нескольким ролям).
- Управляющие действия (create/update/delete) доступны только `owner` и `agronomist`.
- Для `worker` и `observer` страница остаётся доступной в режиме чтения.
