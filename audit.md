...
- **✅ Пункт 6 «Остальное» — «Незначительно» выполнено 2026-07-24.** Backend: B22 (гейт роли
  ...
  компонентов), D16 (RESTRICT на `locations.parent_id` ломает каскад
  accounts→nurseries→locations; orphaning уже закрыт проверкой childrenCount в сервисе), инфра
  B29/B34 (по прежнему решению). Коммит — за пользователем.
...
6. ✅ **«Важно» ВЫПОЛНЕНО 2026-07-24.** Backend B14/B16/B19/B21, frontend F5/F6/F10/F11/F12/F14,
   ...
   Инфра (B20/B29/B34) отложена по решению.
   **«Незначительно» ВЫПОЛНЕНО 2026-07-24** (backend B22–B33 кроме инфра B29/B34; frontend
   ...
   рефактор компонента) и D16 (RESTRICT ломает каскад удаления). Коммит — за пользователем.
