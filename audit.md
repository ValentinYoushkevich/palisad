...
- **✅ Пункт 6 «Остальное» — «Важно» выполнено 2026-07-24.** Закрыты backend B14 (rate-limit
  ...
  снят ещё через F13. Инфра-находки (B20/B29/B34) были отложены — закрыты отдельным заходом
  2026-07-24 (см. блок ниже); «Незначительно» (~36) —
...
- **✅ Пункт 6 «Остальное» — «Незначительно» выполнено 2026-07-24.** Backend: B22 (гейт роли
  на PATCH питомника — requireRole на `/:nurseryId`, контроллерный гейт на `/my`, чтобы owner
  без питомника получал 404, а не 403), B23 (`deleteContainerType`: используемый → soft-delete,
  неиспользуемый → физическое удаление, `countUsedByPlants` осмыслен, контракт приёмки M8#6
  сохранён), B24 (мёртвый `ensureStructureRole` удалён), B25 (select плана «флипнут» —
  `/subscriptions/current` возвращает id подписки, поля плана под родными именами), B26
  (errorHandler мапит Postgres 22P02 → 400 вместо 500), B27 (`crypto.randomBytes` вместо
  `Math.random`), B28 (уникальность numeric_code внутри bulk-партии + `logActivity` для bulk),
  B30 (refresh-cookie `path=/api/auth`), B31 (`requireAuth` сверяет `users.is_active` —
  деактивированный сотрудник → 401, не ждёт истечения токена), B32 (гейт роли restore/
  deleteMovement перенесён в middleware), B33 (алиас `GET /api/plans` вынесен из app.js в
  `plans.router.js`). Frontend: F15 (ограничение роста статик-кэша SW), F16 (модульный синглтон
  онлайн-статуса, годный внутри Pinia-actions), F17 (офлайн `delete_movement` +
  `applyMovementToPlant` + сохранение formData), F18/T15 (мёртвый `services/syncQueue.service.js`
  удалён), F19 (мёртвый токенный код удалён), F20 (поллинг уведомлений стоп офлайн + `error` в
  state), F21 (стрим камеры чистится), F22 (`resetState` чистит `nurseryError`), F24 (lazy
  DataTable не фильтрует серверную страницу повторно), F25 (мёртвые пункты меню + `/health`).
  БД: D9 (сняты избыточные индексы), D10 (индексы под FK), D11 (partial-UNIQUE
  tags/users в питомнике), D12 (одна active-подписка на аккаунт), D13 (CHECK stage_labor_norms),
  D14 (защитный down), D15 (составной FK `accounts→nurseries(account_id,id)`). Тесты: T11 (README
  без хрупкого числа), T12 (cron покрыт, исключения coverage сняты — cleanupCron 100%), T13
  (RBAC-матрица `it.each` + notifications-негативы), T14 (проверки тела ответа — нет
  password_hash, битый токен → 401). Прогоны: backend **300/300** (покрытие
  95.05/86.73/98.03/95.05, пороги 90/80), frontend **53/53**, backend-линт 0 ошибок (4 стилевых
  warning'а). Миграции `20260724180000`–`182000` обратимы, `schema.sql` синхронизирован. 5
  доменов — параллельными агентами (B/C/D добраны оркестратором после обрыва сессии), пересечений
  файлов нет. **Отложено:** F23 (рефактор CatalogPage 532 стр. — риск при слабом покрытии
  компонентов; решение пользователя — рефактор не нужен), D16 (RESTRICT на
  `locations.parent_id` ломает каскад accounts→nurseries→locations; orphaning уже закрыт
  проверкой childrenCount в сервисе), инфра B29/B34 (закрыты позже — см. блок ниже).
  Коммит — за пользователем.
  Заметка эксплуатации: D11/D12 на проде с существующими дублями (активные теги/пользователи с
  одинаковым ключом; более одной active-подписки на аккаунт) потребуют дедупликации перед
  `CREATE UNIQUE INDEX`. (Снята 2026-07-24: прод пуст, дедупликация не требуется.)
- **✅ Инфраструктура (B20/B29/B34) — выполнено 2026-07-24.** B20: graceful shutdown в
  `server.js` (SIGTERM/SIGINT → стоп cron'ов, `server.close`, `db.destroy()`, чистый выход
  через `process.exitCode`, форс-выход по таймауту 10 с) + Dockerfile (node — PID 1 вместо
  npm, `USER node`, `HEALTHCHECK` по `/api/health`, `COPY --chown=node:node`). B29: логгер —
  уровень `info` и в проде (override через `LOG_LEVEL`), в production только stdout (файлы
  `logs/*` — dev-only); morgan в проде — `combined`. B34: docker-compose — креденшалы из env
  (user/db/port с dev-дефолтами, `DB_PASSWORD` обязателен через `:?`-гард), порты
  Postgres/backend привязаны к `127.0.0.1`. Внимание: для `docker compose up` теперь нужен
  корневой `.env` с `DB_PASSWORD` (рядом с docker-compose.yml). Прогоны: backend **303/303**,
  линт 0 ошибок (9 предсуществующих warnings). Коммит — за пользователем.
...
- **B20. Нет graceful shutdown; npm как PID 1.** ✅ **Исправлено 2026-07-24** —
  `server.js`: обработчики SIGTERM/SIGINT (`installShutdownHooks`) — стоп cron'ов
  (`start*Cron` возвращают ScheduledTask), `server.close`, `db.destroy()`, чистый выход через
  `process.exitCode`, форс-выход по таймауту 10 с; Dockerfile: `CMD ["node", "--loader", ...]`
  (node — PID 1), `USER node`, `HEALTHCHECK` через `/api/health` (fetch Node 22),
...
- **B29.** ✅ **Исправлено 2026-07-24** — уровень по умолчанию `info` и в проде
  (override через `LOG_LEVEL`); в production только stdout-транспорт (файлы `logs/*` —
  лишь в dev); morgan в проде — `combined`.
...
- **B34.** ✅ **Исправлено 2026-07-24** — креденшалы через `${DB_*}` подстановку
  (user/db/port с dev-дефолтами, `DB_PASSWORD` обязателен — `:?`-гард), порты Postgres и
  backend привязаны к `127.0.0.1`. Для запуска нужен корневой `.env` с `DB_PASSWORD`
...
- **D16. locations.parent_id ON DELETE SET NULL** ⏸ **Отложено 2026-07-24** — RESTRICT на
  ...
  исключён проверкой `childrenCount` в `locationService.deleteLocation`, поэтому оставлен SET NULL.
  (init:76) — удаление участка молча делает
...
6. ✅ **«Важно» ВЫПОЛНЕНО 2026-07-24.** Backend B14/B16/B19/B21, frontend F5/F6/F10/F11/F12/F14,
   ...
   Инфра (B20/B29/B34) была отложена по решению — закрыта отдельным заходом 2026-07-24.
   **«Незначительно» ВЫПОЛНЕНО 2026-07-24** (backend B22–B33 кроме инфра B29/B34; frontend
   F15–F25 кроме F23; БД D9–D16 кроме D16; тесты T11–T15). Итог: backend **300/300** (покрытие
   95.05/86.73/98.03/95.05), frontend **53/53**, линт 0 ошибок; миграции
   `20260724180000`–`182000` обратимы, `schema.sql` синхронизирован. Отложены F23 (крупный
   рефактор компонента — по решению пользователя не нужен) и D16 (RESTRICT ломает каскад
   удаления). Коммит — за пользователем.
   **Инфра (B20/B29/B34) ВЫПОЛНЕНА 2026-07-24:** graceful shutdown + Dockerfile (node PID 1,
   USER node, HEALTHCHECK), прод-логгер (info, stdout-only, morgan combined), docker-compose
   на env (`DB_PASSWORD` обязателен, порты на 127.0.0.1). Backend **303/303**, линт 0 ошибок.
   На этом все пункты аудита закрыты.

---

## Проверка реализации аудита (2026-07-24, сверка кода со всеми пунктами)

Полная сверка всех ✅-пунктов с фактическим кодом (7 параллельных проверок по доменам:
B1–B10, B11–B21, B22–B36, F1–F14, F15–F25, D1–D15 + schema.sql, T1–T14). Итог: **2 реальные
проблемы, 7 мелких замечаний**; остальные пункты подтверждены — реализация соответствует
заявленному, миграции обратимы, schema.sql синхронизирован (кроме одного дрифта ниже).

**Все 9 замечаний устранены 2026-07-24** (пометки ✅ ниже): backend 303/303 (покрытие
95.06/86.68/98.03/95.06, пороги 90/80), frontend 59/59 (+6 тестов на F13/F5), оба линта 0 ошибок.

### Проблемы (требуют фикса)

- ✅ **Исправлено 2026-07-24** — `applyOperationSideEffects` проверяет `newContainerId` через
  `containerTypeRepo.findById(nurseryId, ...)` (системные допускаются) → 404; тесты в
  `tenantIsolation.test.js` (чужой контейнер → 404, системный → 201).
  **По пункту B5 — кросс-tenant дыра осталась в `transplant`.**
  `backend/src/services/operation.service.js:207-212` — `applyOperationSideEffects` при
  `type='transplant'` пишет `data.newContainerId` напрямую в `plants.container_id` без
  проверки принадлежности контейнера питомнику (валидация только `z.string().uuid()`,
  `operation.validators.js:7`). Для `change_stage` стадия проверяется через
  `stageRepo.findById(nurseryId, ...)` (:218-221), для контейнера аналогичной проверки нет;
  FK в БД не nursery-скоупный. Через `POST .../operations` с `type='transplant'` можно
  привязать растение к container_type чужого питомника, и его имя утечёт через JOIN в
  `findPage` — тот же класс IDOR, который B5 закрыл для `containerId` в create/update растения.

- ✅ **Исправлено 2026-07-24** — фото помечается `failed` только при исчерпании ретраев
  элемента очереди; ветка «фото не найдено» ищет по `getPhotoById` (независимо от статуса)
  и не закрывает элемент, пока запись существует; `retryFailed()` реанимирует и очередь, и
  фото (`resetFailedPhotos`). +5 тестов (useSyncManager.test.js / .photo.test.js).
  **По пункту F13 — транзиентный сбой синка = тихая безвозвратная потеря фото.**
  Ретрай `attach_photo` дырявый: при первом сбое POST `useSyncManager.js:163-165` делает
  `markPhotoFailed(localId)` → `pending_photos.status='failed'`, но элемент очереди остаётся
  `pending` (retries 1<3, `db/syncQueue.service.js:42-45`). Следующий прогон:
  `getPendingPhotos()` возвращает только `status='pending'` (`pendingPhotos.service.js:16-19`)
  → фото «не найдено» → ветка `if (!pending) { markDone(item.id) }`
  (`useSyncManager.js:139-142`) молча помечает элемент очереди синхронизированным. Blob
  остаётся сиротой `failed`, вернуть его нечем: `retryFailed()` сбрасывает только
  `sync_queue`, сброса `pending_photos` `failed→pending` нет. Один транзиентный 500/обрыв
  сети во время синка — фото потеряно навсегда без единого сообщения. Фикс: звать
  `markPhotoFailed` только при переходе элемента очереди в `failed` (и/или сбрасывать статус
  фото в `retryFailed`); ветку «фото не найдено» не схлопывать в `markDone`, если запись
  существует со статусом `failed`.

### Мелкие замечания (не блокеры — на усмотрение)

- ✅ **Исправлено 2026-07-24** (счёт включает soft-deleted; тест «занят только soft-deleted
  растением → мягкое» в dictionary.test.js). **По пункту B23** — `countUsedByPlants` считает
  только живые растения (`whereNull('deleted_at')`): контейнер, на который ссылаются лишь
  soft-deleted растения, удалится физически, FK `ON DELETE SET NULL` молча обнулит их
  `container_id` — restore растения потеряет контейнер.
- ✅ **Исправлено 2026-07-24** (`bulkCreate` переведён на объектный параметр, контроллер
  передаёт `userId`, activity атрибуцируется). **По пункту B28** — bulk-`logActivity`
  пишется без `userId`: атрибуция события отсутствует.
- ✅ **Исправлено 2026-07-24** (колонка добавлена в schema.sql с комментарием).
  **По пунктам B13/D3** — док-дрифт: колонка `subscriptions.expiring_notified_at`
  (миграция `20260724140000`) не отражена в `documentation/schema.sql`.
- ✅ **Исправлено 2026-07-24** (комментарий переписан: тип зажат CHECK'ом, новый тип =
  правка констант + миграция). **По пункту D7** — устаревший комментарий
  `backend/src/constants/notification.constants.js:3` («CHECK-констрейнта на тип намеренно
  нет») противоречит добавленному CHECK `chk_notifications_type`.
- ✅ **Исправлено 2026-07-24** (catch глотает сетевую ошибку, `logout()` резолвится
  `{ok:true}`, редирект выполняется; тест в auth.store.test.js). **По пункту F5** — в
  `logout` вызов `http.post('/auth/logout')` в try/finally без catch: офлайн-выход выполнит
  очистку, но promise отклонится и `router.push('/login')` в accept-ветке не выполнится.
- ✅ **Исправлено 2026-07-24** (экспорт удалён; используемые `SYNC_QUEUE_TYPES`/
  `ALLOWED_SYNC_QUEUE_TYPES` сохранены). **По пункту F18** — остаточный мёртвый экспорт
  `SYNC_QUEUE_STATUSES` (`frontend/src/constants/syncQueue.constants.js`) никем не
  импортируется.
- ✅ **Исправлено 2026-07-24** (точное сравнение со строкой БД:
  `expect(res.body.status).toBe(sub.status)`). **По пункту T7** —
  `backend/tests/subscriptions.test.js:42`: `expect(['trial','active']).toContain(...)` —
  толерантный assert по значению поля тела.
