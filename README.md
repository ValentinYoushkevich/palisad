...
Набор из 160+ интеграционных тестов покрывает модули 2–13 через реальный Express (supertest) и
отдельную тестовую БД `palisad_test` (создаётся и мигрируется автоматически в `tests/globalSetup.js`;
внешний GBIF API замокан). Порог покрытия закреплён в CI и `backend/vitest.config.js` (строки,
функции и стейтменты ≥ 90%, ветки ≥ 80%) — это и есть источник истины; точные проценты смотрите в
отчёте покрытия CI (`npm run test:coverage`). Backend поднимать **не нужно** (`app.js` импортируется
напрямую), только Postgres.
