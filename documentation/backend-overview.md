...
3. **Управление питомником** — создание (с автосозданием owner), редактирование, получение данных,
   список питомников аккаунта и переключение активного (`POST /nurseries/:id/switch`,
   v2 «несколько питомников на аккаунт»).
4. **RBAC** — middleware прав доступа. При нескольких питомниках на аккаунте роли — **per-nursery**
   (`users.nursery_id`): один и тот же человек в разных питомниках — отдельные ряды `users` со своими
   ролями; права вычисляются в контексте активного питомника (роль берётся из токена активного
   питомника). См. `tasks/MultiNurseryRolesDecision.md`.
