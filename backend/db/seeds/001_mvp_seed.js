 ...
 *
 * КАНОНИЧЕСКИЙ ПРОВИЖИНЕР СИСТЕМНЫХ СПРАВОЧНИКОВ (аудит D4): это единственный
 * источник системных справочных ДАННЫХ для всех трёх справочников —
 * movement_types, container_types И production_stages. Ранее production_stages
 * зашивались в миграцию 20260614140000, а movement_types/container_types жили
 * только здесь — справочные данные «разъезжались». Теперь миграции создают
 * только СХЕМУ, а системные строки провижинит этот сид. ЕГО НУЖНО ПРОГОНЯТЬ
 * ПОСЛЕ МИГРАЦИЙ (migrate.latest() затем seed) — иначе свежая БД останется без
